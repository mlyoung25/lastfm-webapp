...
btnBubbleLoad.addEventListener("click", async () => {
  ...
  try {
    ...
    if (!artists.length) { showError("No artists returned for that period."); return; }

    // Show chart area while building
    bubbleChart.hidden = false;
    ...
    const nodes = artists.map((a) => {
      ...
      const radius  = Math.max(30, ((pc / maxPlaycount) ** (1/1.2)) * maxBubbleSize * 0.5) * 2;
      const name    = a.name ?? a["#text"] ?? "";
      const hue     = Array.from(name).reduce((s, c) => s + c.charCodeAt(0), 0) % 360;
      return { name, playcount: pc, radius, x: W/2 + (Math.random()-0.5)*80, y: H/2 + (Math.random()-0.5)*80, imageUrl: null, fallbackColor: `hsl(${hue},58%,42%)` };
    });
    ...
    // Chart ready — show expand button and hint
...
// Clamp bubble count input live so users can't type above 50
bubbleCount.addEventListener("input", () => {
  const v = parseInt(bubbleCount.value, 10);
  if (v > 50) bubbleCount.value = 50;
  if (v < 1)  bubbleCount.value = 1;
});
